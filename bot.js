...
const CONFIG = {
  // Remote debugging endpoint if connecting to your open browser
...
async function sendFacebookMessage(recipient, messageText) {
  ...
  try {
    ...
    // 2. Launch persistent browser (Chrome)
  ...
  if (page.url().includes('login') || (await page.$('input[name="email"]'))) {
    console.log('\n⚠️  Please log in to Facebook in the opened window.');
    console.log('Your session will be saved so you won\'t need to log in again.');
...
if (!recipient || !message) {
  console.log(`
Usage:
  node bot.js "<recipient_name_or_id>" "<message>"

Examples:
  node bot.js "ngozi peter" "This is a test im working on"
  `);
  process.exit(1);
}
